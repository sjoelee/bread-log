...
def simulate_frontend_recipe_creation():
  ...
  frontend_form_data = {
    ...
    "instructions": [
      ...
      {"order": 2, "instruction": "Add levain and salt, mix thoroughly until combined"},
      {
        "order": 3,
        "instruction": "Bulk ferment for 4 hours with coil folds every 30 minutes for first 2 hours",
      },
      {"order": 4, "instruction": "Pre-shape into loose round, rest 30 minutes"},
      {
        "order": 5,
        "instruction": "Final shape into boule, place seam-side up in banneton",
      },
      {"order": 6, "instruction": "Proof overnight in refrigerator (12-18 hours)"},
      {
        "order": 7,
...
def simulate_recipe_update(original_recipe):
  ...
  print("🔧 User changed water amount: 750g → 800g (hydration: 75% → 80%)")
  print()
  ...
  _step_diff = compare_instructions(
  ...
  next_major, next_minor = determine_next_version(
    current_major, current_minor, force_major=False
  )

  print(
    f"📈 Version bump: v{current_major}.{current_minor} → v{next_major}.{next_minor}"
  )
  print()
...
def main():
  ...
  print(
    f"   Version: v{recipe['current_version']['version_major']}.{recipe['current_version']['version_minor']}"
  )
  print(
    f"   Hydration: {recipe['bakers_percentages']['other_ingredients'][0]['percentage']:.1f}%"
  )
  print()
  ...
  print(
    f"   New version: v{update_result['version_major']}.{update_result['version_minor']}"
  )
