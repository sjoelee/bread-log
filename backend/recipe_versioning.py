...
def determine_next_version(
  current_major: int, current_minor: int, force_major: bool = False
) -> Tuple[int, int]:
  """
...
def create_version_summary(ingredient_diff: Dict, step_diff: Dict) -> Dict:
  ...
  summary = {
    ...
    "total_changes": (
      len(ingredient_diff["added"])
      + len(ingredient_diff["removed"])
      + len(ingredient_diff["modified"])
      + len(step_diff["added"])
      + len(step_diff["removed"])
      + len(step_diff["modified"])
...
def calculate_bakers_percentages(ingredients: List[Dict]) -> Dict:
  ...
  for ing in flour_ingredients:
    percentage = (
      (ing["amount"] / total_flour_weight) * 100 if total_flour_weight > 0 else 100
    )
  ...
  for ing in other_ingredients:
    percentage = (
      (ing["amount"] / total_flour_weight) * 100 if total_flour_weight > 0 else 0
    )
