...
class DBConnector:
      ...
      if stretch_folds_json:
        try:
          stretch_folds_data = (
            ...
            else json.loads(stretch_folds_json)
          )
  ...
  def get_dough_make(
    self, date: date, name: str, created_at: datetime
  ) -> Optional[DoughMake]:
    sql = """
        SELECT name, date, 
    ...
    if not res:
      raise DatabaseError(
        f"Make {name} created at {created_at} on {date} doesn't exist"
      )
  ...
  def update_dough_make(
    self, date: date, name: str, created_at: datetime, updates: dict
  ):
    """
    ...
    if "stretch_folds" in updates and updates["stretch_folds"] is not None:
      ...
      for sf in updates["stretch_folds"]:
        ...
        stretch_folds_data.append(
          {"fold_number": fold_number, "timestamp": timestamp_str}
        )
    ...
    set_assignments = [
      sql.SQL("{} = %s").format(sql.Identifier(key)) for key in updates.keys()
    ]
    ...
    try:
      with self.db_pool.get_connection() as conn:
        with conn.cursor() as cur:
          ...
          if cur.rowcount == 0:
            raise DatabaseError(
              f"No dough make found with name {name} created at {created_at} on {date}"
            )
          conn.commit()
    except Exception as e:
    ...
    try:
      with self.db_pool.get_connection() as conn:
        with conn.cursor() as cur:
          ...
          account_makes = []
          for row in results:
            account_makes.append({"display_name": row[0], "key": row[1]})
  ...
  def get_recent_dough_makes(
    self, account_id: UUID, limit: int = 10, offset: int = 0
  ) -> List[DoughMake]:
    """
    ...
    for row in res:
      ...
      if stretch_folds_json:
        try:
          ...
          logger.warning(
            f"Invalid JSON in stretch_folds for make {name}: {stretch_folds_json}, error: {e}"
          )
  ...
  def get_distinct_bread_names(self, account_id: UUID) -> List[dict]:
    ...
    for row in res:
      ...
      distinct_names.append({"name": name, "latest_created_at": latest_created_at})

    return distinct_names

  def add_account_make(
    self, account_id: str, account_name: str, display_name: str, key: str
  ):
