...
def test_recipe_data_structure_compatibility():
  ...
  frontend_recipe_data = {
    ...
    "instructions": [
      ...
      {"order": 2, "instruction": "Add levain and salt, mix thoroughly"},
      {
        "order": 3,
        "instruction": "Bulk ferment for 4 hours with folds every 30 minutes",
      },
      {"order": 4, "instruction": "Pre-shape and rest 30 minutes"},
      {"order": 5, "instruction": "Final shape and proof overnight in fridge"},
      {"order": 6, "instruction": "Bake at 450°F for 45 minutes"},
    ],
  }
  ...
  for i, (frontend_ing, backend_ing) in enumerate(
    zip(frontend_ingredients, backend_ingredients)
  ):
  ...
  water_percentage = next(
    ing["percentage"] for ing in bp["other_ingredients"] if ing["name"] == "water"
  )
...
def test_version_update_workflow():
  ...
  original_recipe = {
    ...
    "instructions": [{"order": 1, "instruction": "Autolyse for 30 minutes"}],
  }
  ...
  for orig_ing, new_ing in zip(
    original_recipe["ingredients"], updated_recipe["ingredients"]
  ):
...
def test_bakers_percentage_calculation():
  ...
  for ing in flour_ingredients:
    ...
    flour_percentages.append(
      {"name": ing["name"], "amount": ing["amount"], "percentage": percentage}
    )
