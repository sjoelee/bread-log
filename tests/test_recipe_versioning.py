...
class TestRecipeVersionCreation:
    ...
    def test_auto_minor_version_bump_on_ingredient_change(self, recipe_version_v1, modified_ingredients):
        ...
        water_ingredient = next(ing for ing in new_version["ingredients"] if ing["name"] == "water")
        assert water_ingredient["amount"] == 800
    
    def test_auto_minor_version_bump_on_ingredient_addition(self, recipe_version_v1, modified_ingredients):
        ...
        olive_oil = next(ing for ing in new_version["ingredients"] if ing["name"] == "olive oil")
    ...
    def test_step_reordering_creates_new_version(self, recipe_version_v1, sample_instructions):
        ...
        assert new_version["instructions"][1]["instruction"] == "Pre-shape and rest 30 minutes"
    
    def test_step_content_modification_creates_new_version(self, recipe_version_v1, modified_instructions):
        ...
        bulk_step = next(step for step in new_version["instructions"] if "bulk ferment" in step["instruction"].lower())
        assert "4 hours at 78°F" in bulk_step["instruction"]


def create_recipe_version(recipe_id: str, ingredients: list, instructions: list, 
                         previous_version=None, description=None, force_major=False):
    """
