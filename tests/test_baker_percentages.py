...
class TestBakerPercentageCalculations:
  ...
  def test_basic_bread_recipe_percentages(self):
    ...
    # Water should be 75%
    water_item = [ing for ing in result["other_ingredients"] if ing["name"] == "water"][
      0
    ]
  ...
  def test_multiple_flour_types_percentages(self):
    ...
    bread_flour = [
      f for f in result["flour_ingredients"] if f["name"] == "bread flour"
    ][0]
    assert bread_flour["percentage"] == 80.0

    # Whole wheat should be 20%
    ww_flour = [
      f for f in result["flour_ingredients"] if f["name"] == "whole wheat flour"
    ][0]
  ...
  def test_no_flour_ingredients_default_behavior(self):
    ...
    salt_item = [ing for ing in result["other_ingredients"] if ing["name"] == "salt"][0]
    assert salt_item["percentage"] == 1.0  # 10/1000 * 100
  ...
  def test_high_hydration_recipe_percentages(self):
    ...
    oil_item = [
      ing for ing in result["other_ingredients"] if ing["name"] == "olive oil"
    ][0]
    ...
    yeast_item = [
      ing for ing in result["other_ingredients"] if ing["name"] == "active dry yeast"
    ][0]
  ...
  def test_preferment_calculations(self):
    ...
    starter_item = [
      ing for ing in result["other_ingredients"] if ing["name"] == "sourdough starter"
    ][0]
  ...
  def test_zero_amount_ingredients(self):
    ...
    sugar_item = [ing for ing in result["other_ingredients"] if ing["name"] == "sugar"][
      0
    ]
...
class TestBakerPercentageIntegration:
  ...
  def test_recipe_creation_includes_correct_percentages(self):
    ...
    recipe_data = {
      ...
      "instructions": [{"order": 1, "instruction": "Mix flour and water"}],
    }
